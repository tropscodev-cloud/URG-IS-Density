...
## **Local Data Assets (not tracked in git)**
`backend/data/` is gitignored — camera footage, model weights, and generated indexes are runtime inputs/outputs, not source. Place these locally before running the pipeline:
- `data/cam1.mp4`, `data/cam2.mp4`, `data/cam3.mp4` — sample camera feeds
- `data/yolov8n.pt` — YOLOv8 nano detection weights ([download from Ultralytics](https://github.com/ultralytics/assets/releases))
- `data/embeddings/faiss.index` — generated automatically on first run of the identity/re-ID pipeline

Also copy `.env.example` to `.env` and fill in `JWT_SECRET` (generate one with `python -c "import secrets; print(secrets.token_urlsafe(48))"`) before starting the API server — it fails fast at startup if unset.
